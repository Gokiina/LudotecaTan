...
public class PageableRequest implements Serializable {
    ...
    public Pageable getPageable() {
        ...
        for (SortRequest sortReq : this.sort) {
            Sort.Order order = new Sort.Order(sortReq.getDirection() != null ? sortReq.getDirection() : Sort.Direction.ASC, sortReq.getProperty());
            orders.add(order);
        }

        Sort sort = Sort.by(orders);
        return PageRequest.of(this.pageNumber, Math.max(this.pageSize, 1), sort);
    }
